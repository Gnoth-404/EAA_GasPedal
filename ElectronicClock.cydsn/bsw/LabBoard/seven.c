...
RC_t SEVEN_SetDP(SEVEN_id_t sevenId, uint8_t value)
{
    ...
    uint8_t patternWithorWithoutDP =0;
    if (value ==1)
    {
        patternWithorWithoutDP = SEVEN__BitPattern[16] | pattern;
    }
    else
    {
        patternWithorWithoutDP = pattern;
    }
    SEVEN_reg_Write(patternWithorWithoutDP);
...
RC_t SEVEN_Chasing(SEVEN_id_t sevenId) {
    // Assuming you have a way to control individual segments,
    // and SEVEN_Set(SEVEN_id_t, uint8_t) sets the individual segments.

    // Constants representing the segments in the order for the chasing effect
    const uint8_t chasingSequence[] = {SEVEN__A, SEVEN__B, SEVEN__C, SEVEN__D, SEVEN__E, SEVEN__F};

    // Duration for each segment to be lit
    const int delayTime = 100; // in milliseconds

    for (uint8_t i = 0; i < sizeof(chasingSequence); ++i) {
        // Turn on the current segment
        SEVEN_reg_Write(chasingSequence[i]);

        // Wait for a bit
        CyDelay(delayTime);

        // Optionally, turn off the segment before moving to the next
        if (SEVEN_Clear(sevenId) != RC_SUCCESS) {
            return RC_ERROR; // Replace with appropriate error code
        }
    }
